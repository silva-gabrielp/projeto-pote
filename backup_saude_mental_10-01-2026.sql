...
--
-- Host: localhost:3306
-- Tempo de geração: 09-Jan-2026 às 23:19
...
-- Procedimentos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_reagir_conteudo` (IN `p_fk_usuario` BIGINT, IN `p_item_tipo` ENUM('atividade','desafio','oficina'), IN `p_fk_item` BIGINT, IN `p_acao` ENUM('like','deslike','visualizacao'))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_registrar_login` (IN `p_fk_dados_acesso` BIGINT, IN `p_foi_login` TINYINT, IN `p_motivo` ENUM('direto','erro','desconhecido','expirado','hack'), IN `p_ip` VARCHAR(64), IN `p_device_info` VARCHAR(300), IN `p_session_id` VARCHAR(191))  BEGIN
...
CREATE TABLE `atividade` (
  ...
  `data_atualizacao` datetime(3) DEFAULT current_timestamp(3) ON UPDATE current_timestamp(3),
...
--
-- Estrutura da tabela `conta_acesso`
--
...
CREATE TABLE `conteudo_reacao` (
  ...
  `fk_item` bigint(20) UNSIGNED NOT NULL,
...
CREATE TABLE `dados_acesso` (
  ...
  `tentativas_login` smallint(5) UNSIGNED NOT NULL DEFAULT 0,
...
CREATE TABLE `dados_profissionais` (
  `id` bigint(20) UNSIGNED NOT NULL,
  `fk_dados_pessoais` bigint(20) UNSIGNED NOT NULL,
...
CREATE TABLE `estatisticas_app` (
  `id` bigint(20) UNSIGNED NOT NULL,
  `numero_dispositivos_sem_autenticacao` int(10) UNSIGNED DEFAULT 0,
  `numero_usuarios` int(10) UNSIGNED DEFAULT 0,
  `numero_profissionais` int(10) UNSIGNED DEFAULT 0,
  `numero_empresas` int(10) UNSIGNED DEFAULT 0,
  `numero_delecoes` int(10) UNSIGNED DEFAULT 0,
  `numero_usuarios_pagos` int(10) UNSIGNED DEFAULT 0,
  `ganhos` int(10) UNSIGNED DEFAULT 0,
  `data_criacao` datetime(3) NOT NULL DEFAULT current_timestamp(3),
  `data_atualizacao` datetime(3) DEFAULT NULL ON UPDATE current_timestamp(3)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `notificacao` (
  ...
  `meio` enum('todos','app','in_app','out_app','web','email','whatsapp') NOT NULL DEFAULT 'app'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `registro_login` (
  ...
  `foi_login` tinyint(1) NOT NULL DEFAULT 1,
...
--
-- Estrutura stand-in para vista `vw_atividade_desafio_usuario`
-- (Veja abaixo para a view atual)
--
CREATE TABLE `vw_atividade_desafio_usuario` (
`conteudo_id` bigint(20) unsigned
,`tipo_conteudo` varchar(9)
,`fk_criador` bigint(20) unsigned
,`fk_usuario` bigint(20) unsigned
,`titulo` varchar(255)
,`descricao` mediumtext
,`tema` varchar(120)
,`data_criacao` datetime(3) /* mariadb-5.3 */
,`data_atualizacao` datetime(3) /* mariadb-5.3 */
,`duracao` bigint(20)
,`tem_video` tinyint(4)
,`link_video` varchar(500)
,`likes` int(11)
,`deslikes` int(11)
,`visualizacoes` int(11)
,`numero_participantes` int(11)
,`data_inicio_usuario` datetime(3) /* mariadb-5.3 */
,`data_fim_usuario` datetime(3) /* mariadb-5.3 */
,`completado` tinyint(4)
,`etapas_feitas` int(11)
,`etapas` longtext
,`imagem` longblob
);
...
--
-- Estrutura stand-in para vista `vw_profissionais_busca`
-- (Veja abaixo para a view atual)
--
CREATE TABLE `vw_profissionais_busca` (
`profissional_id` bigint(20) unsigned
,`nome_completo` varchar(255)
,`estado` varchar(100)
,`cidade` varchar(120)
,`tipo_profissional` varchar(14)
,`e_psicologo` tinyint(1)
,`e_psiquiatra` tinyint(1)
,`e_neuropsicologo` tinyint(1)
,`modalidade_atendimento` enum('presencial','online','ambos')
,`aceita_convenio` tinyint(1)
,`esta_divulgado` tinyint(1)
);

-- --------------------------------------------------------

--
-- Estrutura para vista `vw_atividade_desafio_usuario`
--
DROP TABLE IF EXISTS `vw_atividade_desafio_usuario`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_atividade_desafio_usuario`  AS SELECT `a`.`id` AS `conteudo_id`, 'ATIVIDADE' AS `tipo_conteudo`, `a`.`fk_criador` AS `fk_criador`, `au`.`fk_usuario` AS `fk_usuario`, `a`.`titulo` AS `titulo`, `a`.`sobre` AS `descricao`, `a`.`tema` AS `tema`, `a`.`data_criacao` AS `data_criacao`, `a`.`data_atualizacao` AS `data_atualizacao`, `a`.`duracao` AS `duracao`, `a`.`tem_video` AS `tem_video`, `a`.`link_video` AS `link_video`, `a`.`likes` AS `likes`, `a`.`deslikes` AS `deslikes`, `a`.`visualizacoes` AS `visualizacoes`, `a`.`numero_participantes` AS `numero_participantes`, `au`.`data_inicio` AS `data_inicio_usuario`, `au`.`data_fim` AS `data_fim_usuario`, `au`.`completado` AS `completado`, `au`.`etapas_feitas` AS `etapas_feitas`, `a`.`etapas` AS `etapas`, `a`.`image` AS `imagem` FROM (`atividade` `a` left join `atividade_user` `au` on(`au`.`fk_atividade` = `a`.`id`)) ;

-- --------------------------------------------------------

--
-- Estrutura para vista `vw_profissionais_busca`
--
DROP TABLE IF EXISTS `vw_profissionais_busca`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_profissionais_busca`  AS SELECT `dp`.`id` AS `profissional_id`, `pes`.`nome_completo` AS `nome_completo`, `pes`.`estado` AS `estado`, `pes`.`cidade` AS `cidade`, CASE WHEN `dp`.`e_psicologo` = 1 THEN 'Psicólogo' WHEN `dp`.`e_psiquiatra` = 1 THEN 'Psiquiatra' WHEN `dp`.`e_neuropsicologo` = 1 THEN 'Neuropsicólogo' ELSE 'Outro' END AS `tipo_profissional`, `dp`.`e_psicologo` AS `e_psicologo`, `dp`.`e_psiquiatra` AS `e_psiquiatra`, `dp`.`e_neuropsicologo` AS `e_neuropsicologo`, `dp`.`modalidade_atendimento` AS `modalidade_atendimento`, `dp`.`aceita_convenio` AS `aceita_convenio`, `dp`.`esta_divulgado` AS `esta_divulgado` FROM (`dados_profissionais` `dp` join `dados_pessoais` `pes` on(`pes`.`id` = `dp`.`fk_dados_pessoais`)) ;
...
ALTER TABLE `atividade`
  ...
  ADD KEY `idx_atividade_fk_criador` (`fk_criador`),
  ADD KEY `idx_atividade_data_criacao` (`data_criacao`),
  ADD KEY `idx_atividade_visualizacoes` (`visualizacoes`),
  ADD KEY `idx_atividade_tema` (`tema`),
  ADD KEY `idx_atividade_titulo` (`titulo`);
...
ALTER TABLE `atividade_user`
  ...
  ADD UNIQUE KEY `uq_atividade_user_atividade_usuario` (`fk_atividade`,`fk_usuario`),
  ADD KEY `fk_atividade_user_atividade` (`fk_atividade`),
  ADD KEY `idx_atividade_user_fk_atividade` (`fk_atividade`),
  ADD KEY `idx_atividade_user_fk_usuario` (`fk_usuario`),
  ADD KEY `idx_atividade_user_usuario_completado` (`fk_usuario`,`completado`);
...
--
-- Índices para tabela `conta_acesso`
--
ALTER TABLE `conta_acesso`
  ADD PRIMARY KEY (`id`);

--
-- Índices para tabela `conteudo_reacao`
--
ALTER TABLE `conteudo_reacao`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uq_usuario_item` (`fk_usuario`,`item_tipo`,`fk_item`),
  ADD KEY `idx_item` (`item_tipo`,`fk_item`),
  ADD KEY `idx_usuario` (`fk_usuario`);
...
ALTER TABLE `dados_acesso`
  ...
  ADD UNIQUE KEY `ux_firebase_uid` (`firebase_uid`),
  ...
  ADD UNIQUE KEY `idx_acesso_firebase_uid` (`firebase_uid`),
  ADD KEY `idx_email` (`email`),
  ADD KEY `idx_telefone` (`telefone`),
  ADD KEY `idx_fk_profissional` (`fk_profissional`),
  ADD KEY `idx_fk_empresa` (`fk_empresa`),
  ADD KEY `idx_acesso_deleted_at` (`data_exclusao`),
  ADD KEY `fk_dados_cobranca_usuario` (`fk_user`);
...
ALTER TABLE `dados_cobranca`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_fk_user` (`fk_user`),
  ADD KEY `idx_fk_cartao` (`fk_cartao`);
...
ALTER TABLE `dados_empresariais`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `ux_dados_empresariais_cnpj` (`cnpj`),
  ADD UNIQUE KEY `idx_dados_empresariais_cnpj` (`cnpj`),
  ADD KEY `ix_dados_empresariais_fk_dados_acesso` (`fk_dados_acesso`);
...
ALTER TABLE `dados_pessoais`
  ...
  ADD KEY `idx_fk_dados_acesso` (`fk_dados_acesso`),
  ADD KEY `idx_cpf` (`cpf`),
  ADD KEY `idx_pais_estado` (`pais`,`estado`),
  ADD KEY `fk_dados_pessoais_tipo_plano` (`plano`),
  ADD KEY `idx_dp_nome` (`nome_completo`),
  ADD KEY `idx_dp_cidade` (`cidade`),
  ADD KEY `idx_dp_estado` (`estado`),
  ADD KEY `idx_pessoais_regiao` (`estado`,`cidade`);
...
ALTER TABLE `dados_profissionais`
  ...
  ADD KEY `fk_dp_atendimento` (`fk_atendimento`),
  ADD KEY `idx_dados_profissionais_fk_dados_pessoais` (`fk_dados_pessoais`),
  ADD KEY `idx_profissionais_tipo` (`e_psicologo`,`e_psiquiatra`,`e_neuropsicologo`),
  ADD KEY `idx_profissionais_convenio` (`aceita_convenio`);
...
ALTER TABLE `desafio`
  ...
  ADD KEY `idx_desafio_fk_criador` (`fk_criador`),
  ADD KEY `idx_desafio_data_criacao` (`data_criacao`),
  ADD KEY `idx_desafio_visualizacoes` (`visualizacoes`);
...
ALTER TABLE `desafio_user`
  ...
  ADD UNIQUE KEY `uq_desafio_user_desafio_usuario` (`fk_desafio`,`fk_usuario`),
  ADD KEY `fk_desafio_user_desafio` (`fk_desafio`),
  ADD KEY `idx_desafio_user_fk_desafio` (`fk_desafio`),
  ADD KEY `idx_desafio_user_fk_usuario` (`fk_usuario`),
  ADD KEY `idx_desafio_user_usuario_completado` (`fk_usuario`,`completado`);
...
ALTER TABLE `pote`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uk_pote_nome` (`nome`);
...
ALTER TABLE `usuario_sem_autenticacao`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `idx_identificador_unique` (`identificador`);
...
--
-- AUTO_INCREMENT de tabela `conta_acesso`
--
ALTER TABLE `conta_acesso`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `conteudo_reacao`
--
ALTER TABLE `conteudo_reacao`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
-- Limitadores para a tabela `conteudo_reacao`
--
ALTER TABLE `conteudo_reacao`
  ADD CONSTRAINT `fk_cr_reacao_usuario` FOREIGN KEY (`fk_usuario`) REFERENCES `dados_acesso` (`id`) ON DELETE CASCADE;
...
ALTER TABLE `dados_acesso`
  ...
  ADD CONSTRAINT `fk_dadosacesso_dadosemp` FOREIGN KEY (`fk_empresa`) REFERENCES `dados_empresariais` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_dadosacesso_dadosprof` FOREIGN KEY (`fk_profissional`) REFERENCES `dados_profissionais` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
ALTER TABLE `dados_cobranca`
  ...
  ADD CONSTRAINT `fk_dadoscobranca_dados_acesso` FOREIGN KEY (`fk_user`) REFERENCES `dados_acesso` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;

--
-- Limitadores para a tabela `dados_empresariais`
--
ALTER TABLE `dados_empresariais`
  ADD CONSTRAINT `fk_dados_empresariais_dados_acesso` FOREIGN KEY (`fk_dados_acesso`) REFERENCES `dados_acesso` (`id`) ON UPDATE CASCADE,
...
ALTER TABLE `dados_pessoais`
  ...
  ADD CONSTRAINT `fk_dados_pessoais_dados_acesso` FOREIGN KEY (`fk_dados_acesso`) REFERENCES `dados_acesso` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
ALTER TABLE `dados_profissionais`
  ...
  ADD CONSTRAINT `fk_dados_profissionais_dados_pessoais` FOREIGN KEY (`fk_dados_pessoais`) REFERENCES `dados_pessoais` (`id`) ON UPDATE CASCADE,
...
ALTER TABLE `erro_app`
  ...
  ADD CONSTRAINT `fk_erroapp_criador` FOREIGN KEY (`fk_criador`) REFERENCES `conta_acesso` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
--
ALTER TABLE `registro_login`
  ADD CONSTRAINT `fk_registrologin_dadosacesso` FOREIGN KEY (`fk_dados_acesso`) REFERENCES `dados_acesso` (`id`) ON DELETE CASCADE;
